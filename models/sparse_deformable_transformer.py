...
from models.ops.modules import MSDeformAttn2
...
class DeformableTransformer(nn.Module):
    ...
    def forward(self, srcs, masks, pos_embeds, query_embed=None):
        ...
        # print(f"Initial src_flatten token count: {src_flatten.shape[1]}")
        ###########
        ...
        #     print(f"Top-k selected token count: {backbone_topk_proposals.shape[1]}")
        # if sparse_token_nums is not None:
        #     print(f"Sparse token counts per batch: {sparse_token_nums}")
        ###########
        # encoder
        if self.encoder:       
        ...
        ###########
        # decoder
        # print(f"Tgt shape (queries): {tgt.shape}")  # 查看 queries 数量
...
class DeformableTransformerEncoderLayer(nn.Module):
    def __init__(self,
        ...
        # self attention
        self.self_attn = MSDeformAttn2(d_model, n_levels, n_heads, n_points)
...
class DeformableTransformerEncoder(nn.Module):
    ...
    def forward(self, src, spatial_shapes, level_start_index, valid_ratios, 
        ...
        if sparsified_keys:
            ...
            # 稀疏化后的 token 数量
            # print(f"Tokens after sparsification: {topk_inds.shape[1]}")
        ...
        for lid, layer in enumerate(self.layers):
            # 打印每一层的 token 数量
            # if sparsified_keys:
            #     print(f"Layer {lid}: Token count before attention: {tgt.shape[1]}")
            # else:
            #     print(f"Layer {lid}: Token count before attention: {output.shape[1]}")
            ...
            if sparsified_keys:                
                ...
                # # 打印每一层后的 token 数量
                # print(f"Layer {lid}: Token count after sparse reduction: {output.shape[1]}")        
            else:
                output = tgt
...
class DeformableTransformerDecoderLayer(nn.Module):
    def __init__(self, d_model=256, d_ffn=1024, dropout=0.1, activation="relu",
        ...
        # cross attention
        self.cross_attn = MSDeformAttn2(d_model, n_levels, n_heads, n_points)
        self.dropout1 = nn.Dropout(dropout)
        self.norm1 = nn.LayerNorm(d_model)
